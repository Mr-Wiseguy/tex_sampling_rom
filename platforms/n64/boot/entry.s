...
.clear_bytes:
    ...
    lui   $sp, %hi(idleThreadStack) # Set the high half of the stack pointer to that of the idle thread stack
    addiu $t2, %lo(init) # Get the low half of the init function address
    jr    $t2 # Jump to the init function
     addiu $sp, %lo(idleThreadStack) # Set the low half of the stack pointer to that of the idle thread stack
